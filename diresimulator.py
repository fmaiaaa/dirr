...
Versão: 2.5 (Design Premium & Funcionalidade Reforçada)
=============================================================================
"""
...
# =============================================================================
# 1. CONFIGURAÇÕES E DADOS (SIMULADOS COM BASE NAS TABELAS ENVIADAS)
# =============================================================================

def carregar_dados_sistema():
    # Simulando a "TABELA FINANCIAMENTOS" enviada nos snippets
    rendas = np.arange(1700, 12100, 100)
    data_finan = []
    for r in rendas:
        # Lógica simplificada: Financiamento aprox 50-60x a renda dependendo das flags
        data_finan.append({
            'Renda': r,
...
class MotorVendas:
    ...
    def obter_financiamento(self, renda, social, cotista):
        ...
        # Determina coluna baseado nas chaves
...
def aba_painel_vendas(df_finan, df_estoque, df_politicas):
    ...
    with col_input:
        ...
        with st.container():
            ...
            renda = st.number_input("Renda Familiar Bruta (R$)", min_value=1700.0, value=3500.0, step=100.0)
            
            c1, c2 = st.columns(2)
            with c1: social = st.toggle("Fator Social", value=False)
            with c2: cotista = st.toggle("Cotista FGTS", value=True)
            
            f_sug, s_sug = motor.obter_financiamento(renda, social, cotista)
            
            st.markdown("---")
            finan_final = st.number_input("Financiamento Ajustado (R$)", value=f_sug)
            fgts = st.number_input("FGTS + Subsídio Total (R$)", value=s_sug)
            
            st.markdown("---")
            classif = st.selectbox("Classificação de Crédito", options=df_politicas['CLASSIFICAÇÃO'].unique(), index=2)
            
            unidade = st.selectbox("Unidade Disponível", 
                                  options=df_estoque[df_estoque['Status'] == 'Disponível']['Identificador'])
            
            btn_calcular = st.button("🚀 Gerar Fluxo de Pagamento")
            st.markdown('</div>', unsafe_allow_html=True)

    with col_result:
        if btn_calcular:
            dados_unidade = df_estoque[df_estoque['Identificador'] == unidade].iloc[0]
            vv = dados_unidade['Valor de Venda']
            res = motor.calcular_simulacao(vv, finan_final, fgts, classif)
            
            st.markdown(f"### 📊 Resultado: {unidade}")
            ...
            fig = go.Figure(data=[go.Pie(
                ...
                values=[finan_final, fgts, res['ps_financiado'], res['ato_complementar']],
                hole=.5,
            ...
            <div style="background: white; padding: 20px; border-radius: 15px; border-left: 5px solid {'#10b981' if res['ato_complementar'] == 0 else '#f59e0b'};">
                <strong>Análise Técnica:</strong> <span class="{status_class}">{status_txt}</span><br><br>
                O GAP total é de <b>R$ {res['gap']:,.2f}</b>. <br>
                Capacidade de parcelamento ({classif}): <b>{res['parcelas']}x</b>.<br>
                Entrada mínima necessária (Ato): <b>R$ {res['ato_complementar']:,.2f}</b>.
            </div>
            """, unsafe_allow_html=True)
        else:
            st.info("Aguardando preenchimento dos dados para gerar o relatório...")
...
def main():
    ...
    with tabs[2]:
        ...
        st.warning("⚠️ Valores de Pro Soluto que excedam o percentual máximo da classificação serão automaticamente convertidos em Ato (Entrada).")
