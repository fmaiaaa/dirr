...
Versão: 19.0 (Correção Final de Sintaxe, Tela Duplicada e KeyError)
=============================================================================
"""
...
def aba_simulador_automacao(df_finan, df_estoque, df_politicas):
    ...
    if st.session_state.passo_simulacao == 'input':
        ...
        with col_center:
            nome = st.text_input("Nome do Cliente", value=st.session_state.dados_cliente.get('nome', ""), key="in_nome_v19")
            renda = st.number_input("Renda Bruta Familiar (R$)", min_value=1.0, value=st.session_state.dados_cliente.get('renda', 3500.0), step=100.0, key="in_renda_v19")
            ranking_options = df_politicas['CLASSIFICAÇÃO'].unique().tolist() if not df_politicas.empty else ["DIAMANTE"]
            ranking = st.selectbox("Ranking do Cliente", options=ranking_options, index=0, key="in_rank_v19")
            politica_ps = st.selectbox("Política de Pro Soluto", ["Direcional", "Emcash"], key="in_pol_v19")
            social = st.toggle("Fator Social", value=st.session_state.dados_cliente.get('social', False), key="in_soc_v19")
            cotista = st.toggle("Cotista FGTS", value=st.session_state.dados_cliente.get('cotista', True), key="in_cot_v19")
            
            if st.button("🚀 Avançar para Visão Financeira", type="primary", use_container_width=True, key="btn_s1_v19"):
    ...
    # --- ETAPA 2 ---
        ...
        if st.button("🏢 Ver Produtos Viáveis", type="primary", use_container_width=True, key="btn_s2_v19"):
            st.session_state.passo_simulacao = 'guide'; st.rerun()
        st.write("")
        if st.button("⬅️ Editar Dados", use_container_width=True, key="btn_edit_v19"):
    ...
    # --- ETAPA 3 ---
        ...
        if df_viaveis.empty:
            ...
            if st.button("⬅️ Voltar", use_container_width=True, key="btn_v_err_v19"): st.session_state.passo_simulacao = 'potential'; st.rerun()
        else:
            ...
            with tab_rec:
                emp_rec = st.selectbox("Filtrar por Empreendimento:", options=["Todos"] + sorted(df_viaveis['Empreendimento'].unique().tolist()), key="sel_emp_v19")
                ...
                st.markdown("---")
                if st.button("💰 Ir para Fechamento", type="primary", use_container_width=True, key="btn_fech_v19"):
                    st.session_state.passo_simulacao = 'payment_flow'; st.rerun()
                st.write("")
                if st.button("⬅️ Voltar ao Potencial", use_container_width=True, key="btn_pot_v19"): 
    ...
    # --- ETAPA 4 ---
        ...
        emp_def = st.selectbox("Escolha o Empreendimento:", options=sorted(df_estoque['Empreendimento'].unique()), key="sel_emp_f_v19")
        unidades = df_estoque[(df_estoque['Empreendimento'] == emp_def) & (df_estoque['Status'] == 'Disponível')]
        
        if not unidades.empty:
            u_id = st.selectbox("Escolha a Unidade:", options=unidades['Identificador'].unique(), key="sel_uni_f_v19")
            u = unidades[unidades['Identificador'] == u_id].iloc[0]
            
            f_u = st.number_input("Financiamento (R$)", value=float(d['finan_estimado']), key="fin_u_v19")
            fgts_u = st.number_input("FGTS + Subsídio (R$)", value=float(d['fgts_sub']), key="fgt_u_v19")
            ps_max_v = u['Valor de Venda'] * d['perc_ps']
            ps_u = st.number_input("Pro Soluto Total (R$)", value=float(ps_max_v), key="ps_u_v19")
            
            # Correção do KeyError e do Valor Padrão
            parc = st.number_input("Parcelas Pro Soluto", min_value=1, max_value=d['prazo_ps_max'], value=d['prazo_ps_max'], key="parc_u_v19")
            ...
            st.markdown(f"""
                ...
                <div class="fin-box" style="background:#f8fafc; border-top: 5px solid #2563eb;"><b>Parcela PS:</b> R$ {v_parc:,.2f} ({comp_r*100:.1f}% da Renda)</div>
            """, unsafe_allow_html=True)
            ...
            if saldo_e > 0:
                ...
                st.number_input("Ato (R$)", value=saldo_e/4, key="a1_v19")
                st.number_input("30 dias (R$)", value=saldo_e/4, key="a2_v19")
                st.number_input("60 dias (R$)", value=saldo_e/4, key="a3_v19")
                st.number_input("90 dias (R$)", value=saldo_e/4, key="a4_v19")
        
        st.markdown("---")
        if st.button("⬅️ Voltar", use_container_width=True, key="btn_v_guide_v19"): st.session_state.passo_simulacao = 'guide'; st.rerun()
        if st.button("👤 Novo Cliente", use_container_width=True, key="btn_new_c_v19"): st.session_state.passo_simulacao = 'input'; st.rerun()
...
def carregar_dados_sistema():
    try:
        ...
        try:
            ...
            # Filtro para evitar preços irreais (como os 664k ou zeros)
            df_estoque = df_estoque[
                (df_estoque['Valor de Venda'] > 100000) & 
                (df_estoque['Valor de Venda'] < 1000000) & 
                (df_estoque['Empreendimento'].notnull())
            ].copy()
...
def aba_simulador_automacao(df_finan, df_estoque, df_politicas):
    ...
    if st.session_state.passo_simulacao == 'input':
        ...
        _, col_center, _ = st.columns([1, 2, 1])
        with col_center:
            nome = st.text_input("Nome do Cliente", value=st.session_state.dados_cliente.get('nome', ""), key="in_nome_v18_1")
            renda = st.number_input("Renda Bruta Familiar (R$)", min_value=1.0, value=st.session_state.dados_cliente.get('renda', 3500.0), step=100.0, key="in_renda_v18_1")
            ranking_options = df_politicas['CLASSIFICAÇÃO'].unique().tolist() if not df_politicas.empty else ["EMCASH"]
            ranking = st.selectbox("Ranking do Cliente", options=ranking_options, index=0, key="in_rank_v18_1")
            politica_ps = st.selectbox("Política de Pro Soluto", ["Direcional", "Emcash"], key="in_pol_v18_1")
            social = st.toggle("Fator Social", value=st.session_state.dados_cliente.get('social', False), key="in_soc_v18_1")
            cotista = st.toggle("Cotista FGTS", value=st.session_state.dados_cliente.get('cotista', True), key="in_cot_v18_1")
            
            if st.button("🚀 Avançar para Visão Financeira", type="primary", use_container_width=True, key="btn_s1_v18_1"):
                finan, sub = motor.obter_enquadramento(renda, social, cotista)
                class_b = 'EMCASH' if politica_ps == "Emcash" else ranking
                politica_row = df_politicas[df_politicas['CLASSIFICAÇÃO'] == class_b].iloc[0]
                limit_ps_r = politica_row['FX_RENDA_1'] if renda < politica_row['FAIXA_RENDA'] else politica_row['FX_RENDA_2']
    ...
    # --- ETAPA 2 ---
        ...
        if st.button("🏢 Ver Produtos Viáveis", type="primary", use_container_width=True, key="btn_s2_v18_1"):
            st.session_state.passo_simulacao = 'guide'; st.rerun()
        st.write("")
        if st.button("⬅️ Editar Dados", use_container_width=True, key="btn_edit_v18_1"):
            st.session_state.passo_simulacao = 'input'; st.rerun()
    ...
    # --- ETAPA 3 ---
        ...
        if df_viaveis.empty:
            st.error("❌ Nenhuma unidade viável no estoque para este perfil.")
            if st.button("⬅️ Voltar", use_container_width=True, key="btn_v_err_v18_1"): st.session_state.passo_simulacao = 'potential'; st.rerun()
        else:
            with st.expander("🏢 Empreendimentos Disponíveis", expanded=True):
                emp_counts = df_viaveis.groupby('Empreendimento').size().to_dict()
                for emp, qtd in emp_counts.items():
            ...
            tab_rec, tab_list = st.tabs(["⭐ Recomendações", "📋 Estoque Completo"])
            with tab_rec:
                emp_rec = st.selectbox("Filtrar por Empreendimento:", options=["Todos"] + sorted(df_viaveis['Empreendimento'].unique().tolist()), key="sel_emp_v18_1")
                df_filt = df_viaveis if emp_rec == "Todos" else df_viaveis[df_viaveis['Empreendimento'] == emp_rec]
                df_filt = df_filt.sort_values('Valor de Venda', ascending=False)
                ...
                st.markdown("---")
                if st.button("💰 Ir para Fechamento", type="primary", use_container_width=True, key="btn_fech_v18_1"):
                    st.session_state.passo_simulacao = 'payment_flow'; st.rerun()
                st.write("")
                if st.button("⬅️ Voltar ao Potencial", use_container_width=True, key="btn_pot_v18_1"): 
                    st.session_state.passo_simulacao = 'potential'; st.rerun()
    ...
    # --- ETAPA 4 ---
        ...
        d = st.session_state.dados_cliente
        st.markdown(f"### 📑 Etapa 4: Fechamento Financeiro")
        emp_def = st.selectbox("Escolha o Empreendimento:", options=sorted(df_estoque['Empreendimento'].unique()), key="sel_emp_f_v18_1")
        unidades = df_estoque[(df_estoque['Empreendimento'] == emp_def) & (df_estoque['Status'] == 'Disponível')]
        
        if not unidades.empty:
            u_id = st.selectbox("Escolha a Unidade:", options=unidades['Identificador'].unique(), key="sel_uni_f_v18_1")
            u = unidades[unidades['Identificador'] == u_id].iloc[0]
            
            f_u = st.number_input("Financiamento (R$)", value=float(d['finan_estimado']), key="fin_u_v18_1")
            fgts_u = st.number_input("FGTS + Subsídio (R$)", value=float(d['fgts_sub']), key="fgt_u_v18_1")
            
            ps_max_valor = u['Valor de Venda'] * d['perc_ps']
            ps_u = st.number_input("Pro Soluto Total (R$)", value=float(ps_max_valor), key="ps_u_v18_1")
            st.markdown(f'<p class="inline-ref">Limite do Ranking ({int(d["perc_ps"]*100)}%): R$ {ps_max_valor:,.2f}</p>', unsafe_allow_html=True)
            
            # Correção do KeyError: Usando 'prazo_ps_max' que foi definido na Etapa 1
            parc = st.number_input("Quantidade de Parcelas Pro Soluto", min_value=1, max_value=d['prazo_ps_max'], value=d['prazo_ps_max'], key="parc_u_v18_1")
            ...
            if saldo_e > 0:
                ...
                st.number_input("Valor Ato (R$)", value=saldo_e/4, key="ato_1_v18_1")
                st.number_input("30 dias (R$)", value=saldo_e/4, key="ato_2_v18_1")
                st.number_input("60 dias (R$)", value=saldo_e/4, key="ato_3_v18_1")
                st.number_input("90 dias (R$)", value=saldo_e/4, key="ato_4_v18_1")
        
        st.markdown("---")
        if st.button("⬅️ Voltar", use_container_width=True, key="btn_back_v18_1"): st.session_state.passo_simulacao = 'guide'; st.rerun()
        if st.button("👤 Novo Cliente", use_container_width=True, key="btn_new_v18_1"): st.session_state.passo_simulacao = 'input'; st.rerun()
