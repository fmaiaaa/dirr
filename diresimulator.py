...
Versão: 52.3 (Atualização de Tabelas e Lógica de Faixas F2/F3/F4)
=============================================================================
"""
...
def carregar_dados_sistema():
    try:
        ...
        try:
            # 1. Carrega os dados brutos de estoque conforme novo formato
...
class MotorRecomendacao:
    ...
    def obter_enquadramento(self, renda, social, cotista, valor_venda=275000):
        """
        Lógica atualizada: Seleciona a coluna baseada no valor da unidade (Faixas).
        F2: até 275k | F3: até 350k | F4: até 500k
        """
        if self.df_finan.empty: return 0.0, 0.0
        
        # Define a Faixa (Sufixo da Coluna)
        if valor_venda <= 275000:
            faixa_sufixo = "_F2"
        elif valor_venda <= 350000:
            faixa_sufixo = "_F3"
        elif valor_venda <= 500000:
            faixa_sufixo = "_F4"
        else:
            faixa_sufixo = "_F4" # Cap default
        ...
        # Constrói o nome da coluna dinamicamente conforme o formato solicitado
        c_finan = f"Finan_Social_{s_suf}_Cotista_{c_suf}{faixa_sufixo}"
        c_sub = f"Subsidio_Social_{s_suf}_Cotista_{c_suf}{faixa_sufixo}"
...
def aba_simulador_automacao(df_finan, df_estoque, df_politicas):
    ...
    if st.session_state.passo_simulacao == 'input':
        ...
        ranking_options = [r for r in df_politicas['CLASSIFICACAO'].unique().tolist() if r != "EMCASH"] if not df_politicas.empty else ["DIAMANTE"]
        ...
        if st.button("Avançar para Valor Potencial de Compra", type="primary", use_container_width=True, key="btn_s1_v23"):
            if not nome.strip():
                ...
                # Na Etapa 1, calculamos o enquadramento base para Faixa 2 para mostrar o potencial
                finan, sub = motor.obter_enquadramento(renda, social, cotista, valor_venda=275000)
                class_b = 'EMCASH' if politica_ps == "Emcash" else ranking
                politica_row = df_politicas[df_politicas['CLASSIFICACAO'] == class_b].iloc[0]
    ...
    # --- ETAPA 2 ---
        ...
        if df_pot.empty:
            ...
            # Para o potencial geral, mostramos baseado na renda e nos limites de financiamento
        ...
        with m1: st.markdown(f'<div class="card"><p class="metric-label">Financiamento Base (F2)</p><p class="metric-value">R$ {fmt_br(d["finan_estimado"])}</p></div>', unsafe_allow_html=True)
        with m2: st.markdown(f'<div class="card"><p class="metric-label">FGTS + Subsídio Base (F2)</p><p class="metric-value">R$ {fmt_br(d["fgts_sub"])}</p></div>', unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class="card" style="border-top: 4px solid {COR_AZUL_ESC}; background: #ffffff; min-height: 120px;">
                ...
                <p style="font-size: 0.7rem; color: {COR_TEXTO_MUTED};">Nota: O financiamento pode variar conforme a faixa de preço da unidade (F2, F3 ou F4).</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    # --- ETAPA 3 ---
        ...
        if df_disp_total.empty:
            ...
            # Lógica de Viabilidade Unidade a Unidade (Calculando enquadramento específico para cada preço)
            def avaliar_viabilidade_unid(row_venda):
                # Busca enquadramento específico para a faixa dessa unidade
                f_unid, s_unid = motor.obter_enquadramento(d['renda'], d['social'], d['cotista'], row_venda)
                poder_unid, ps_unid = motor.calcular_poder_compra(d['renda'], f_unid, s_unid, d['perc_ps'], row_venda)
                return poder_unid, s_unid, f_unid
            
            res_v = df_disp_total['Valor de Venda'].apply(avaliar_viabilidade_unid)
            df_disp_total['Poder_Compra'] = [x[0] for x in res_v]
            df_disp_total['Sub_Espec'] = [x[1] for x in res_v]
            df_disp_total['Fin_Espec'] = [x[2] for x in res_v]
        ...
        with tab_rec:
            if df_viaveis.empty:
                st.info("Nenhuma unidade atende ao perfil.")
            else:
                ...
                if not df_filt_rec.empty:
                    ...
                    with c1: st.markdown(f'<div class="recommendation-card" style="border-top: 4px solid {COR_AZUL_ESC};"><b>TOP</b><br><small>{r100["Empreendimento"]}</small><br>{r100["Identificador"]}<br><span class="price-tag">R$ {fmt_br(r100["Valor de Venda"])}</span></div>', unsafe_allow_html=True)
                    with c2: st.markdown(f'<div class="recommendation-card" style="border-top: 4px solid {COR_VERMELHO};"><b>MÉDIO</b><br><small>{r90["Empreendimento"]}</small><br>{r90["Identificador"]}<br><span class="price-tag">R$ {fmt_br(r90["Valor de Venda"])}</span></div>', unsafe_allow_html=True)
                    with c3: st.markdown(f'<div class="recommendation-card" style="border-top: 4px solid {COR_AZUL_ESC};"><b>ACESSO</b><br><small>{r75["Empreendimento"]}</small><br>{r75["Identificador"]}<br><span class="price-tag">R$ {fmt_br(r75["Valor de Venda"])}</span></div>', unsafe_allow_html=True)
            ...
            if df_disp_total.empty:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Valor de Venda": st.column_config.TextColumn("Venda (R$)", width="medium"),
                        "Poder_Compra": st.column_config.TextColumn("Teto (R$)", width="medium"),
        ...
        with col_sel2:
            if unidades_disp.empty:
                ...
                uni_escolhida_id = st.selectbox("Escolha a Unidade:", options=unidades_disp['Identificador'].unique(), 
                                                format_func=lbl_uni, key="sel_uni_guide_v26")

        st.write("")
        if st.button("Avançar para Fechamento Financeiro", type="primary", use_container_width=True, key="btn_fech_v26"):
            if uni_escolhida_id:
                u_sel = unidades_disp[unidades_disp['Identificador'] == uni_escolhida_id].iloc[0]
                # Recalcula enquadramento exato para a unidade escolhida
                fin_u, sub_u = motor.obter_enquadramento(d['renda'], d['social'], d['cotista'], u_sel['Valor de Venda'])
                
                st.session_state.dados_cliente.update({
                    'unidade_id': uni_escolhida_id,
                    'empreendimento_nome': emp_escolhido,
                    'imovel_valor': u_sel['Valor de Venda'],
                    'finan_estimado_u': fin_u,
                    'fgts_sub_u': sub_u
                })
    ...
    # --- ETAPA 4 ---
        ...
        v_venda = d.get('imovel_valor')
        
        st.markdown(f'<div class="custom-alert">Unidade: {u_id} - {d.get("empreendimento_nome")} (R$ {fmt_br(v_venda)})</div>', unsafe_allow_html=True)
        
        f_u = st.number_input("Financiamento Bancário", value=float(d['finan_estimado_u']), step=1000.0, key="fin_u_v23")
        st.markdown(f'<p class="inline-ref">Calculado para Faixa: R$ {fmt_br(d["finan_estimado_u"])}</p>', unsafe_allow_html=True)
        
        fgts_u = st.number_input("FGTS + Subsídio", value=float(d['fgts_sub_u']), step=1000.0, key="fgt_u_v23")
        st.markdown(f'<p class="inline-ref">Calculado para Faixa: R$ {fmt_br(d["fgts_sub_u"])}</p>', unsafe_allow_html=True)
        
        ps_max_real = v_venda * d['perc_ps']
        ...
        saldo_e = v_venda - f_u - fgts_u - ps_u
        ...
        with fin1: st.markdown(f"""<div class="fin-box" style="border-top: 6px solid {COR_AZUL_ESC};"><b>IMÓVEL</b><br>R$ {fmt_br(v_venda)}</div>""", unsafe_allow_html=True)
        with fin2: st.markdown(f"""<div class="fin-box" style="border-top: 6px solid {COR_VERMELHO};"><b>MENSALIDADE PS</b><br>R$ {fmt_br(v_parc)}</div>""", unsafe_allow_html=True)
        ...
        if comp_r > d['limit_ps_renda']:
            st.warning(f"Parcela excede o limite de {d['limit_ps_renda']*100:.0f}% da renda.")
        ...
        if saldo_e > 0:
            ...
            soma_e = st.session_state.ato_1 + st.session_state.ato_2 + st.session_state.ato_3 + st.session_state.ato_4
            if abs(soma_e - saldo_e) > 1.0:
                st.error(f"Soma difere do saldo de entrada.")
    ...
    # --- ETAPA 5 ---
        ...
        d = st.session_state.dados_cliente
        st.markdown(f"### Resumo - {d.get('nome')}")
        
        if PDF_ENABLED:
            pdf_data = gerar_resumo_pdf(d)
            if pdf_data:
                st.download_button("Baixar Resumo PDF", data=pdf_data, file_name=f"Resumo Direcional - {d.get('nome')}.pdf", mime="application/pdf", use_container_width=True)
        ...
        st.markdown(f"""<div class="summary-body"><b>Empreendimento:</b> {d.get('empreendimento_nome')}<br>
            <b>Unidade:</b> {d.get('unidade_id')}<br><b>Valor:</b> <span style="color: {COR_VERMELHO};">R$ {fmt_br(d.get('imovel_valor', 0))}</span></div>""", unsafe_allow_html=True)

        st.markdown(f'<div class="summary-header">FINANCIAMENTO</div>', unsafe_allow_html=True)
        st.markdown(f"""<div class="summary-body">Financiamento: R$ {fmt_br(d.get('finan_usado'))}<br>Sub/FGTS: R$ {fmt_br(d.get('fgts_sub_usado'))}<br>Pro Soluto: R$ {fmt_br(d.get('ps_usado'))} ({d.get('ps_parcelas')}x R$ {fmt_br(d.get('ps_mensal'))})</div>""", unsafe_allow_html=True)

        st.markdown(f'<div class="summary-header">ENTRADA</div>', unsafe_allow_html=True)
        st.markdown(f"""<div class="summary-body"><b>Total: R$ {fmt_br(d.get('entrada_total'))}</b><br>Ato: {fmt_br(d.get('ato_final'))} | 30: {fmt_br(d.get('ato_30'))} | 60: {fmt_br(d.get('ato_60'))} | 90: {fmt_br(d.get('ato_90'))}</div>""", unsafe_allow_html=True)
