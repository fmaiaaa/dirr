    ...
    try:
        ...
        try:
            ...
            mapa_estoque = {
                ...
                'Valor de Venda': 'Valor de Venda', # ALTERAÇÃO SOLICITADA
                ...
                'PS Bronze': 'PS_Bronze',
                'PS Aço': 'PS_Aco',
                'Previsão de expedição do habite-se': 'Data Entrega', # ALTERAÇÃO SOLICITADA
                'Área privativa total': 'Area',
                'Tipo Planta/Área': 'Tipologia',
                'Endereço': 'Endereco',
                'Folga Volta o Caixa': 'Volta_Caixa' # ALTERAÇÃO SOLICITADA
            }
        ...
        <body style="font-family: 'Helvetica', sans-serif; color: #333; background-color: #f9f9f9; margin: 0; padding: 20px;">
            <div style="max-width: 600px; margin: auto; background-color: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                <div style="background-color: #ffffff; padding: 30px; text-align: center; border-bottom: 4px solid #e30613;">
                    <img src="{URL_FAVICON_RESERVA}" width="80" style="margin-bottom: 10px;">
                </div>
        ...
        <body style="font-family: 'Arial', sans-serif; color: #333; background-color: #eee; margin: 0; padding: 20px;">
            <div style="max-width: 650px; margin: auto; background-color: #fff; padding: 30px; border: 1px solid #ccc;">
                <div style="text-align: center; margin-bottom: 20px;">
                      <img src="{URL_FAVICON_RESERVA}" width="80">
                </div>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 0.9em;">
                    <tr style="background-color: #f2f2f2;">
                        <td width="50%" style="padding: 8px; border: 1px solid #ddd;">Valor Venda (VCM)</td>
                        <td width="50%" style="padding: 8px; border: 1px solid #ddd; text-align: right; color: #e30613;"><b>R$ {val_venda}</b></td>
                    </tr>
                    <tr>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 0.9em;">
                    <tr style="background-color: #f2f2f2;">
                        <td width="50%" style="padding: 8px; border: 1px solid #ddd;">Entrada Total</td>
                        <td width="50%" style="padding: 8px; border: 1px solid #ddd; text-align: right; color: #002c5d;"><b>R$ {entrada}</b></td>
                    </tr>
                    <tr>
...
def dialog_novo_cliente(motor):
    ...
    # FORMATACAO AUTOMATICA CPF ON LOAD/RERUN
    if curr_cpf:
        curr_cpf = formatar_cpf_saida(curr_cpf)
    ...
    with st.form("form_cadastro"):
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        submitted = st.form_submit_button("Confirmar e Salvar", type="primary", use_container_width=True)
    ...
    if submitted:
        ...
        # NÃO AVANÇA MAIS AUTOMATICAMENTE - RERUN para atualizar página inicial com dados carregados
        st.rerun()
    ...
    if filtered_df.empty:
        ...
        for idx, row in filtered_df.iterrows():
            ...
            if st.button(label_btn, key=f"btn_cli_{idx}", use_container_width=True):
                ...
                # NÃO AVANÇA MAIS AUTOMATICAMENTE - RERUN para atualizar página inicial
                st.rerun()
...
def aba_simulador_automacao(df_finan, df_estoque, df_politicas, df_cadastros):
    ...
    if passo == 'gallery':
        ...
        # Barra horizontal para seleção de empreendimentos
        if not lista_produtos:
            st.warning("Nenhum produto cadastrado na galeria.")
        else:
            sel_galeria = st.selectbox("Selecione o Empreendimento", lista_produtos, label_visibility="collapsed")
            st.markdown("---")
            ...
            meta = CATALOGO_PRODUTOS[sel_galeria]
            ...
            # Organize all images in one ordered list: Map, Masterplan, Plants, Images
            imagens_raw = meta.get("imagens", [])
            all_imgs_sorted = []
            link_ficha = None
            
            # Categories buckets
            cats = {"MAPA": [], "MASTERPLAN": [], "PLANTAS": [], "OUTROS": []}
            ...
            for item in imagens_raw:
                ...
                if "MAPA" in nome_up: cats["MAPA"].append((item, full_link))
                elif "MASTERPLAN" in nome_up: cats["MASTERPLAN"].append((item, full_link))
                elif "PLANTA" in nome_up: cats["PLANTAS"].append((item, full_link))
                else: cats["OUTROS"].append((item, full_link))
            
            # Flatten in order
            final_list = cats["MAPA"] + cats["MASTERPLAN"] + cats["PLANTAS"] + cats["OUTROS"]
            
            # Serialize for JS
            js_links = json.dumps([x[1] for x in final_list])
            
            # Render all in one scrolling wrapper
            if final_list:
                st.markdown("##### Imagens do Empreendimento")
                html = '<div class="scrolling-images">'
                for idx, (item, link) in enumerate(final_list):
                    thumb_src = link
                    ...
                    <div style="position: relative; display: inline-block;">
                        <img src="{thumb_src}" alt="{item["nome"]}" title="{item["nome"]}" onclick='openGallery({js_links}, {idx})' style="display: block;">
                        <div onclick='openGallery({js_links}, {idx})' style="position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.6); color: white; padding: 5px; border-radius: 4px; cursor: pointer;">⤢</div>
                        <div style="position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,44,93,0.8); color: white; padding: 5px; font-size: 0.8rem; text-align: center; border-radius: 0 0 12px 12px;">{item["nome"]}</div>
                    </div>
                    '''
                html += '</div>'
                st.markdown(html, unsafe_allow_html=True)
    ...
    # --- ABA ANALYTICS (SECURE TAB - ALTAIR) ---
        ...
        if v_fin > 0 and p_fin > 0:
            ...
            # CORREÇÃO DO ERRO ALTAIR: Garantir tipos primitivos
    ...
    # --- ETAPA 1: INPUT ---
        ...
        # CSS para os botões estilizados
        st.markdown(f"""
        <style>
        .home-card-btn {{
            display: flex;
        ...
        /* Estilo das barras de navegação inferiores */
        .nav-bar-bottom {{
            display: flex;
            justify-content: space-between;
            gap: 20px;
        ...
        with col_new:
            # Botão CADASTRAR estilizável via st.button é limitado, mas podemos usar HTML/CSS custom + st.button invisivel se quisermos, 
            # mas vamos manter st.button com o hack CSS acima aplicado a eles
        ...
        # Hack CSS para aplicar o estilo .home-card-btn nos botões acima
        # Como não podemos adicionar classes diretamente no st.button, usamos o seletor de atributo
        st.markdown(f"""
        <script>
            // Tentar injetar classe via JS se possível, ou CSS específico
        </script>
        <style>
        /* Seleciona botões específicos pelo texto para aplicar o estilo de card grande */
        ...
        /* Aumenta o tamanho dos botões específicos */
        ...
        # Se houver cliente carregado, mostrar opções de navegação
        if 'dados_cliente' in st.session_state and 'nome' in st.session_state.dados_cliente:
            st.markdown(f"""
            <div style="margin-top: 40px; padding: 20px; background: #f8fafc; border-radius: 12px; border: 1px solid {COR_AZUL_ESC};">
                <h4 style="margin: 0 0 15px 0; text-align: center;">Cliente Ativo: {st.session_state.dados_cliente['nome']}</h4>
                <p style="text-align: center; color: #64748b; font-size: 0.9rem;">Selecione a próxima etapa:</p>
            </div>
            """, unsafe_allow_html=True)
            
            nav_c1, nav_c2 = st.columns(2)
            with nav_c1:
                if st.button("Obter Recomendação de Unidades", use_container_width=True, type="primary"):
                    st.session_state.passo_simulacao = 'guide'
                    st.rerun()
            with nav_c2:
                if st.button("Ir para Escolha de Unidade", use_container_width=True):
    ...
    # --- ETAPA 2: RECOMENDAÇÃO ---
        ...
        # Helper para value=None se 0.0 para mostrar placeholder
        ...
        ps_max_real = 0.0
        # Recuperação da Volta ao Caixa (Se existir)
        volta_caixa_val = 0.0
        ...
        if 'unidade_id' in d and 'empreendimento_nome' in d:
             ...
             if not row_u.empty:
                 ...
                 # Volta ao Caixa
                 volta_caixa_val = row_u.get('Volta_Caixa', 0.0)
        ...
        # Calculo do GAP Final (Saldo a Cobrir)
        # GAP = ValorVenda - Finan - FGTS - PS - Entradas - VoltaCaixa
        gap_final = u_valor - f_u_input - fgts_u_input - ps_input_val - total_entrada_cash - volta_caixa_val
        
        st.markdown("<hr style='margin: 10px 0;'>", unsafe_allow_html=True)
        
        # Exibir Volta ao Caixa se houver
        if volta_caixa_val > 0:
             st.info(f"Dedução Volta ao Caixa aplicada: R$ {fmt_br(volta_caixa_val)}")
