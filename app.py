...
from simulador_dv.core.pro_soluto_comparador import (
    menor_prazo_parcelas_ps_respeitando_j8,
    metricas_pro_soluto,
...
from simulador_dv.salesforce_streamlit import lookup_ranking_salesforce_cached
...
def dialog_novo_cliente(motor):
    ...
    with st.form("form_cadastro"):
        ...
        st.caption(
            "Se o Salesforce estiver configurado (secrets), o ranking pode ser ajustado automaticamente pelo CPF ao confirmar."
        )
    ...
    if submitted:
        ...
        _cpf_d = limpar_cpf_visual(cpf_formatado)
        if len(_cpf_d) == 11:
            r_sf, _ = lookup_ranking_salesforce_cached(_cpf_d)
            if r_sf and r_sf in rank_opts:
                ranking_final = r_sf
                if hasattr(st, "toast"):
                    st.toast(f"Ranking definido pelo Salesforce: {r_sf}", icon="✅")
        ...
        if not lista_produtos:
            ...
            for aba, prod_key in zip(tabs_produtos, lista_produtos):
                with aba:
                    ...
                    # Imagens (campanhas): miniaturas quadradas, sem título; clique = lightbox
                    ...
                    for cat in ["MAPA", "MASTERPLAN", "PLANTAS", "IMAGENS ILUSTRATIVAS"]:
                        ordered_pairs.extend(categorias[cat])
                    ordered_links = [link for _, link in ordered_pairs]
                    json_links = json.dumps(ordered_links)

                    def _thumb_src_for_grid(link: str) -> str:
                        if "drive.google.com/uc?export=view&id=" in link:
                            rest = link.split("id=", 1)[1]
                            fid = rest.split("&")[0].split("?")[0]
                            return f"https://drive.google.com/thumbnail?id={fid}&sz=w200"
                        return link

                    if ordered_pairs:
                        st.markdown("### Campanhas Comerciais")
                        parts = ['<div class="scrolling-images">']
                        for local_idx, (_item, link) in enumerate(ordered_pairs):
                            ts = _thumb_src_for_grid(link)
                            parts.append(
                                f'<img class="gallery-thumb" src="{ts}" alt="" loading="lazy" '
                                f"onclick='openGallery({json_links}, {local_idx})'>"
                            )
                        parts.append("</div>")
...
<p style="font-weight: 700; font-size: 1.1rem; margin: 0; color: {COR_AZUL_ESC};">Cadastrar Novo Cliente</p></div>""", unsafe_allow_html=True)
            ...
            if not nome.strip(): st.error("Por favor, informe o Nome do Cliente.")
                ...
                if not validar_cpf(cpf_val): st.error("CPF Inválido.")
                    ...
                    st.session_state.dados_cliente.update({
                        ...
                        'social': social, 'cotista': cotista, 'ranking': ranking_final, 'politica': politica_ps,
    ...
    # --- ETAPA 3: RECOMENDAÇÃO (usa valores reais do fechamento) ---
        ...
        j8_mps = float(mps.get("parcela_max_j8") or 0)
        pol_ui = str(d.get("politica", "Direcional"))
        ps_limite_base = float(mps.get("ps_max_efetivo", 0) or 0)

        with col_ps_parc:
            if "parc_ps_key" not in st.session_state:
                st.session_state["parc_ps_key"] = d.get("ps_parcelas", min(60, parc_max_ui))
            ...
            st.markdown(
                f'<span class="inline-ref">Prazo máx. parcelas: {parc_max_ui} meses (política {pol_prazo} × app {prazo_cap_app})</span>',
                unsafe_allow_html=True,
            )

        ps_limite_ui2 = ps_limite_base
        _ps_max_in = float(ps_limite_ui2) if ps_limite_ui2 and ps_limite_ui2 > 0 else None
        with col_ps_val:
            ps_input_val = st.number_input(
                "Pro Soluto Direcional",
                value=st.session_state.get("ps_u_key", _num_val("ps_u_key", 0.0)),
                key="ps_u_key",
                min_value=0.0,
                max_value=_ps_max_in if _ps_max_in is not None else None,
                step=1000.0,
                format="%.2f",
                placeholder="0,00",
            )
            if ps_input_val is None:
                st.session_state["ps_u_key"] = 0.0
                ps_input_val = 0.0
                st.rerun()
            st.session_state.dados_cliente["ps_usado"] = float(ps_input_val) if ps_input_val is not None else 0.0
            ref_text_ps = f"Limite máximo de Pro Soluto: R$ {fmt_br(ps_limite_ui2)}"
            st.markdown(
                f'<div class="inline-ref" style="background-color: transparent; padding: 0; font-family: inherit; font-size: 0.72rem; color: {COR_AZUL_ESC}; margin-top: -12px; margin-bottom: 15px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; display: block; opacity: 0.9;">{ref_text_ps}</div>',
                unsafe_allow_html=True,
            )

        n_min_j8 = None
        if float(ps_input_val or 0) > 0 and j8_mps > 0:
        ...
        v_parc = parcela_ps_para_valor(
            ...
            pol_ui,
            _prem,
            parcela_max_j8=j8_mps if j8_mps > 0 else None,
        )
        ...
        st.markdown(
            f'<div style="margin-top: -8px; margin-bottom: 15px; font-size: 0.9rem; font-weight: 600; color: {COR_AZUL_ESC}; text-align: center;">Mensalidade PS: R$ {fmt_br(v_parc)} ({parc}x)</div>',
            unsafe_allow_html=True,
        )
        st.caption(
            f"Parcela máx. (J8): R$ {fmt_br(mps.get('parcela_max_j8', 0))} · G14: R$ {fmt_br(mps.get('parcela_max_g14', 0))}"
        )
        if float(ps_input_val or 0) > 0 and j8_mps > 0:
            if n_min_j8 is not None:
                st.caption(
                    f"Sugestão de intervalo: entre **{n_min_j8}** e **{parc_max_ui}** parcelas a prestação fica dentro do teto J8 "
                    f"(R$ {fmt_br(j8_mps)}/mês). Com **{n_min_j8}x** já é possível respeitar o teto — evite ir direto a **{parc_max_ui}x** sem necessidade."
                )
                if parc < n_min_j8:
                    st.warning(
                        f"Com {parc} parcelas a prestação tende a ultrapassar o teto J8. Use pelo menos {n_min_j8} parcelas "
                        "ou reduza o valor do Pro Soluto."
                    )
            else:
                st.warning(
                    "Com este valor de Pro Soluto e o prazo máximo permitido, a prestação pode ultrapassar o teto J8. Reduza o PS ou ajuste o perfil."
                )
        st.caption(
            "Direcional: fluxo B43/CE; Emcash: I5 (PMT×(1+E1)). A parcela não ultrapassa o teto J8."
        )
